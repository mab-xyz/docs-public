...
## GET /v1/analysis/tx-risk-hash/{tx_hash}

Fetch an existing confirmed mainnet transaction by hash, simulate it at the block it was mined, and return a risk assessment in the exact same shape as `tx-risk-raw`.

This endpoint is designed for post-confirmation analysis workflows: given a transaction that is already on-chain, return the same structured risk verdict you would get from `tx-risk-raw` if you had submitted the raw bytes.

### What the endpoint does

- **Fetches the transaction** by hash from the configured Ethereum node using `eth_getTransactionByHash`.
- **Simulates the transaction** using `trace_call` at the block the transaction was mined, so the state matches what actually executed.
- **Sets the interaction history window** to `[0, blockNumber-1]` by default — checking prior interactions strictly before this transaction, the same semantics as `simulate_from_tx`.
- **Extracts every touched contract** including contracts reached through internal calls and `DELEGATECALL`s.
- **Checks source-code availability** for each touched contract. Any unverified contract is flagged as dangerous.
- **Checks interaction novelty** using on-chain scan history. A *first-time interaction* means the sender or the root contract has never called this address before.
...
### tx-risk-hash — analyze an existing confirmed transaction

**Mainnet tx:** `0x0af5a6d2d8b49f68dcfd4599a0e767450e76e08a5aeba9b3d534a604d308e60b`

Pass a confirmed transaction hash. The API fetches the transaction, replays it at its block, and returns the same response shape as `tx-risk-raw`.

```bash
curl https://api.mab.xyz/v1/analysis/tx-risk-hash/0x0af5a6d2d8b49f68dcfd4599a0e767450e76e08a5aeba9b3d534a604d308e60b \
  -H "X-API-Key: $MAB_API_KEY"
```
  ...
  "details": [
    {
      ...
      "depth": 0,
      "types": null,
      "interaction_first_time": { "contract_direct": false, "contract_transitive": false },
      "interaction_state": { "contract_direct": "FOUND", "contract_transitive": "FOUND" },
      "is_eip7702": false,
      "delegate_address": null
    }
  ]
